...
class EnglishGangUser(HttpUser):
    ...
    access_tokens = {"student": None, "teacher": None, "manager": None}
    ...
    def authenticate(self, user_type):
        ...
        if self.access_tokens[user_type] is None:
            ...
            with self.client.post(
                "/api/token",
                data=credentials,  # Правильный формат для отправки формы
                catch_response=True,
                name="Авторизация",
            ) as response:
    ...
    def get_student_profile(self):
        ...
        with self.client.get(
            "/api/me", headers=headers, catch_response=True, name="GET student profile"
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def get_students_list(self):
        ...
        with self.client.get(
            ...
            if response.status_code == 200:
                ...
                response.failure(
                    f"Ошибка получения списка студентов: {response.status_code}"
                )
    ...
    def update_student_info(self):
        ...
        with self.client.get(
            ...
            headers=headers,
            catch_response=True,
            name="GET student for update",
        ) as response:
            if response.status_code != 200:
                response.failure(
                    f"Не удалось получить данные студента: {response.status_code}"
                )
                return
            ...
            update_data = {
                ...
                "last_name": student_data["additional_info"].get(
                    "last_name", "Student"
                ),
                ...
                "level": "B"
                + str(random.randint(1, 2)),  # Случайно меняем уровень между B1 и B2
            ...
            with self.client.put(
                ...
                if put_response.status_code == 200:
                    ...
                    put_response.failure(
                        f"Ошибка обновления студента: {put_response.status_code}"
                    )
