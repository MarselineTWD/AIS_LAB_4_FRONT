...
// Загрузка списка преподавателей (публичная информация)
async function loadTeachers() {
    try {
        console.log('Загрузка публичного списка преподавателей...');
        const response = await fetch('/api/teachers/public');

        if (!response.ok) {
            throw new Error(`Ошибка при загрузке преподавателей: ${response.status}`);
        }

        const teachers = await response.json();
        console.log(`Загружено ${teachers.length} преподавателей`);

        displayTeachers(teachers);
    } catch (error) {
        console.error('Ошибка загрузки преподавателей:', error);
        document.getElementById('teachersList').innerHTML =
            '<div class="error-message">Не удалось загрузить список преподавателей. Пожалуйста, попробуйте позже.</div>';
    }
}
...
async function addStudent(event) {
    ...
    console.log('Отправка данных нового студента:', studentData.first_name, studentData.last_name);
